...
- 子查询，适用于查询类似于，评分最高的值，属于那部手机？这种情况；
- 聚合函数直接转对象，数组。

# 二、GROUP BY

前面介绍的聚合函数，默认把所有的数据，分成了一组：
- 比如 `AVG()`、`MAX()`...，都是将所有的结果看成一组，来计算的；

如果希望划分多个组：比如：计算华为、苹果、小米、等等不同品牌手机，分别的平均价格呢？
...
- 刻意有一些商品数据的品牌，在 `brands` 表中，是没有添加的；并且 `brands` 表中，也添加了一些不存在的手机品牌；
- 实际开发中，经常要处理类似于这样的情况。
...
如果希望可以修改，需要修改外键 `ON DELETE` 或者 `ON UPDATE` 时，对应的操作，可以设置如下几个值：
...
4.重新添加外键，设置 `ON UPDATA`、`ON DELETE` 为 `CASCADE`。
...
如果，在查询到产品的同时，希望显示对应的品牌相关的信息，

因为数据是存放在两张表中，所以这个时候，就需要进行多表查询。

默认多表查询的结果。直接通过如下 SQL 查询查询，是什么效果呢？
...
- 但是发现，`products` 表中，外键值为 `NULL` 的数据，丢失了，
- 因为只能过滤满足条件的数据。
...
在多表查询时，使用 **SQL JOINS** 操作，希望获取到的，是左边所有的数据（以左表为主），使用左连接：
...
在多表查询时，使用 SQL JOINS 操作，希望获取到的，是右边所有的数据（以右表为主），使用右连接：
...
事实上，内连接，是表示，左边的表，和右边的表，都有对应的数据关联： 
...
- SQL 语句二：`WHERE` 条件，代表的是，先计算出笛卡尔乘积，再进行 `WHERE` 条件的帅选；
...
- 如果用内连接，查询的学生没有选课，那么什么也查不出来，所以要用左连接。
