...
Nods 有 http 模块，通常使用基于该模块的框架，如 express、koa，开发服务 i 其。
...
创建服务器对象，是通过 `http.createServer` 来完成的
...
上面我们已经看到，创建 Server 时会传入一个回调函数，这个回调函数在 被调用时会传入两个参数：

- `request` 请求对象，包含请求相关的信息；本质上是一个可读流
- `response` 响应对象，包含我们要发送给客户端的信息；本质上是一个可写流

## 2.开启服务器

server 通过 `listen` 方法，来开启服务器；并且在某一个主机的端口上，监听网络请求：

也就是说，当通过 [ip]:[port] 的方式发送请求，到监听的 Web 服务器上时；就可以对其进行相关的处理；
...
- `host`：主机， 通常可以传入:
  ...
  - `127.0.0.1`：回环地址（Loop Back Address），表达的意思是“主机自己发出去的包，直接被自己接收”；
    ...
    - 而回环地址，在“网络层”直接就被获取到了，是不会经过“数据链路层”和“物理层“的；
...
> 【注意】：
>
> 一般要开启服务器，要监听 `(1024，65535]` 之间的端口。否则可能会覆盖系统的某些特殊服务端口。
>
> 通常操作系统使用两个字节，表示端口；
>
> - 即最大端口是 `255 * 255 = 6553`6，即 `0 - 65535` 之间的端口。
...
node 开启一个服务器，如果修改了其中的代码逻辑，需要手动重启服务器，才能生效。

安装一个依赖 _nodemon_(node monitor 的缩写) 插件，用于 node 服务器的自动重启。
...
客户端发送的请求的地址中，经常携带一些额外的参数：

比如这个请求地址：`http://localhost:8000/login?name=zzt&password=123`

url 的值是 `/login?name=why&password=123`；处理步骤如下：
...
const server = http.createServer((req, res) => {
  ...
  // 方式二（推荐）：
  const queryInfo = new URLSearchParams(queryString) // Entries 数据结构
...
> 【补充】：在框架（如 express）中，封装了 method 和 url 结合的处理。

# 八、body 参数的获取

现有一个创建用户的接口：

请求路径 url 为 `/users`；请求方式 method 为 `POST`；请求携带数据 `username`和`password`；

请求在 body 中携带数据，

- 而 `reuqest` 对象中，没有 body 请求体；
- 需要通过监听 `reqest` 的 `"data"` 事件获取 body 请求体。

`request` 对象，本质上是一个 readable 可读流。
...
在 postmen 中，选择默认的授权方式，token 会被自动放入请求头中；
...
- `res.writeHead`：同时写入 header 和 status 状态码；
