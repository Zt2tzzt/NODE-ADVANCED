...
JavaScript 可以直接处理图片吗？
- 事实上在网页端，图片是交给浏览器来处理的；
- JavaScript 或者 HTML，只是负责告诉浏览器一个图片的地址；
- 浏览器负责获取这个图片，并且最终讲这个图片渲染出来；

对于服务器来说，要处理的本地文件类型相对较多;
- 比如某一个保存文本的文件，使用 GBK（早期 Windows 常用）进行编码，那么必须读取该文件的二进制数据，再通过 GKB 编码，转换成对应的文字；
- 比如读取的是一张图片（二进制数据），再通过某些手段对图片数据进行二次的处理（裁剪、格式转换、旋转、添加滤镜）。
- 比如在 Node 中通过 TCP 建立长连接，TCP 传输的是字节流，需要将数据转成字节，再传入流中，并且需要知道传输字节的大小（客户端需要根据大小来判断读取多少内容）；

> 【补充】：Node 中有一个 *Sharp* 库，用于读取图片或者传入图片的 Buffer，对其进行处理；

# 二、Buffer 和二进制关系

我们会发现，对于前端开发来说，通常很少会和二进制直接打交道；

但是对于服务器端，很多时候，必须直接去操作二进制的数据；

所以 Node 为了可以方便开发者完成更多功能，在全局提供给了一个类“`Buffer`“；

`Buffer` 中存储的是二进制数据，可以将 `Buffer` 看成是一个存储二进制的数组；

这个数组中的每一项，可以保存 8 位二进制： 如 `0000 0000`，即一个字节。
...
>
> 所以 r、g、b，在计算机中，本质上都是用一个字节存储的；

为什么一个字节，是 8 位二进制数字呢？
- 在计算机中，很少的情况会直接操作一位二进制，因为一位二进制存储的数据是非常有限；
- 所以通常会将 8 位合在一起作为一个单元，这个单元称之为一个**字节（byte）**；
...
我们已知，Buffer 相当于是一个字节的数组，数组中的每一项元素，对应一个字节的大小：
...
03-Node类-Buffer\01-Buffer和字符串的转换.js
...
创建一个 8 位长度的 Buffer，里面所有的数据默认为 `00`；

03-Node类-Buffer\02-buffer的其它创建方式.js
...
03-Node类-Buffer\03-从文件中读取buffer.js
...
而是默认先申请一个 8 * 1024 个字节大小的内存，也就是 8kb

如果一次性申请超过 8kb 的内存，Node 会直接申请相应的内存大小。

[Node源码](https://github.com/nodejs/node/blob/main/lib/buffer.js)
...
我们已知，可以使用 `readFile` 或者 `writeFile` 方法，读写文件；这种方式虽然简单直接，但是无法控制一些细节的操作；
- 比如：从什么位置开始读、读到什么位置、一次性读取多少个字节；
- 比如：读到某个位置后，暂停读取，某个时刻恢复继续读取等等；
- 比如：这个文件非常大，像一个视频文件，一次性全部读取并不合适；

04-Node中的流-Stream\01-可读流的基本使用.js
...
- 比如：http 模块的 Request 和 Response 对象；
...
这里我们通过 `fs` 的操作，着重介绍 Writable、Readable。
...
04-Node中的流-Stream\02-可读流的其它事件.js
...
04-Node中的流-Stream\03-可写流的使用过程.js
...
我们会发现，使用 `createWriteStream` 时，并不能直接监听到 `"close"` 事件： 

- 这是因为：写入流在打开后是不会自动关闭的； 

- 必须手动关闭，来告诉 Node 已经写入结束了；Node 会发出一个 `"finish"` 事件的；

写入流一个非常常用的方法是 `end`：该方法相当于做了两步操作： 

1. 写入传入的数据；
2. 调用 `close` 方法；

读取可以监听到 `end` 事件，写入仅仅能监听到 `close` 事件。

# 十、pipe 方法

通过程序代码，实现文件的拷贝，有三种方式：

方式一：尝试通过代码，一次性读取文件，并写入文件，以实现赋值文件的效果。

04-Node中的流-Stream\04-文件的拷贝流操作.js
