...
计算机中，所有的内容：文字、数字、图片、音频、视频，最终都会使用二进制来表示。
...
> 【补充】：Node 中有一个 _Sharp_ 库，用于读取图片，或者传入图片的 Buffer，对其进行处理；

## 二、Buffer 和二进制关系

对于前端开发来说，很少会和二进制直接打交道；
...
- 在计算机中，很少的情况，直接操作一位二进制，因为它存储的数据是非常有限的；
- 所以，通常会将 8 位二进制合在一起，作为一个单元，这个单元称之为一个**字节（byte）**；
- 有这样的换算关系：`1byte = 8bit`，`1kb=1024byte`，`1M=1024kb`；`1G=1024MB`
...
`buffer.toString` 方法，默认会用 `utf8` 编码，将 buffer 转成字符串。
...
- 这里做的事情是，判断 8kb 内存中，剩余的长度，是否还足够填充这个字符串；
- 如果不足够，那么就要通过 `createPool` 创建新的空间；
- 如果够，就直接使用，但是之后要进行 `poolOffset` 的偏移变化；
...
- 比如：读到某个位置后，暂停读取，某个时刻恢复读取；
- 比如：文件非常大时，像一个视频文件，一次性全部读取并不合适；
