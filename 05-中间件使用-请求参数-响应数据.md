...
处理请求的逻辑比较多时，可拆分到多个中间件中，调用 `nwxt` 方法，执行下一个中间件。
...
app.post('/login', (req, res, next) => {
  ...
  if (req.body.username === 'zzt' && dataInfo.password === '123') isLogin = true
...
上述案例中，将 body 请求体中携带的 json 类型参数解析的逻辑，进行了封装。

事实上，express 框架提供的中间件 `express.json()`，用于解析 body 请求体中的 json 数据。
...
以下是 GET 请求传递参数的两种方式，express 都提供了处理方法。
...
记录每次客户端发送请求的日志，使用官方开发的第三方库 `morgan`；
...
使用 `multer` 模块，解析 POST 请求中 body 请求体中的 formData 类型的数据。
