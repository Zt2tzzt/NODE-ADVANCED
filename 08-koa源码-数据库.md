...
- 之后可以利用 SQL 语句，在多张表中查询所需的数据；

**非关系型数据库**有：MongoDB、Redis、Memcached、HBse 等；

- 非关系型数据库的英文其实是“Not only SQL”即“不仅仅是关系型数据库”，也简称为 NoSQL；
- 相对关系型数据库而言，比较简单，存储数据也会更加自由（甚至我们可以直接将一个复杂的 json 对象直接塞入到数据库中）；
- NoSQL 是基于 Key-Value 的对应关系，并且查询的过程中不需要经过 SQL 解析；

如何在开发中选择他们呢？具体的选择，根据不同的项目进行综合的分析：
- 目前在公司进行后端开发（Node、Java、Go 等），还是以**关系型数据库**为主；
- 用到**非关系型数据库**，在爬取大量的数据，或收集大量无固定结构的数据时时，会比较常见；
- 非关系型数据库，相较于关系型数据库
  - 存储的形式随意简单。
  - 不好进行精确地细化地操作。
  - 通常会有大量冗余信息。
...
[下载地址](https://dev.mysql.com/downloads/mysql/)，根据自己的操作系统下载即可；

Windows 推荐下载 MSI 的版本，不用做多余的配置；Mac 推荐下载DMG的版本；在安装过程中会配置一些环境变量；
...
安装完成后，打开 MySQL 命令行工具。输入配置的密码，即可连接到 MySQL.
...
有两种方式来连接：两种方式的区别在于是直接输入密码，还是另起一行以密文的形式输入；
...
- 终端中查看所有的数据库或者表，非常的不直观和不方便；
- 等等...


在实际开发中，可以借助于一些 GUI 工具来连接数据库，在 GUI 工具中操作就会非常方便。
