...
- 很难以合适的方式，组织数据（数据库中，多张表之前的关系组织更合理）；
...
- **表 (Table)**：表是关系型数据库中存储数据的主要结构，它由多行多列组成，类似于电子表格。
- **记录 (Record / Row)**：表中的每一行称为一个记录，每个记录代表一组相关的数据，例如一个订单或一个客户。
- **字段 (Field / Column)**：表中的每一列称为一个字段，它表示记录中的一个数据点，例如客户的姓名或订单的日期。
...
- 相对于关系型数据库，比较简单，存储数据也会更加自由（甚至可以将一个复杂的 json 对象，直接塞入到数据库中）；
- NoSQL 是基于 Key-Value 的对应关系，并且查询的过程中，不需要经过 SQL 解析；

对于非关系型或 NoSQL 数据库，术语会根据数据库的类型（文档，键值，列式，图等）有所变化。以最常见的**文档型数据库**（如 MongoDB）为例：

- **集合 (Collection)**：等同于关系型数据库的 "表"，它包含许多文档。
- **文档 (Document)**：等同于关系型数据库的 "记录"，每个文档都包含一组键值对。
- **字段 (Field)**：通常指文档中的 "键" 或 "键值对"，等同于关系型数据库的 "字段"。
...
- 在爬取大量的数据，或收集大量无固定结构的数据时，用**非关系型数据库**，会比较方便；
...
Mac 推荐下载 DMG 的版本；在安装过程中，会配置一些环境变量；
...
有两种方式来连接：区别在于，是直接输入密码，还是另起一行，以密文的形式输入密码；
