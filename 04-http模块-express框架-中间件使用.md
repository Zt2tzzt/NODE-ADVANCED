...
真实开发中，后端（不管是 Node 还是 Java）原生处理文件上传非常复杂繁琐，一般会借助于插件完成。

为了了解文件上传的完整过程，在此做一个文件原生上传的演示。

在客户端，如浏览器，一般都是通过表单（form）的形式，进行上传；`<input type="file">`
...
由上述案例可知，上床的图片比较小，可一次性读取到，如果上传的图片比较大，会分成几次读取。上面的案例中，就分成了四次。

但保存下来的文件，无法打开，因为数据流中有很多多余的信息，不应该被保存进图片文件。

## 2.正确示例

不能把读取到的数据，全部写入保存下来，需要去除一些多余的数据。步骤会有点繁琐。

1. 读取图片这样的二进制数据时，不要设置编码为 `utf8`，而是 `binary`；

- 这样，读取时会使用 Ascll 编码，对每个字节进行转换。
- 如果解码出来的是文本，就能看到内容；
...
现在发送网络请求，一般不用原生表单携带数据，而是用于上传文件，

那是早期后端渲染时，JSP 会采用原生表单携带数据发送网络请求。

> 【注意】：在用 liverServer 本地测试时，上传图片后可能会刷新，这是因为写入的图片，可能保存在了 liverServer 开启 Web 服务的目录下。

# 三、Web 服务器框架

我们已知，http 内置模块可来搭建 Web 服务器，为什么还要使用框架？
- 原生 http 在进行很多处理时，较为复杂；
- URL 处理、Method 处理、参数处理、逻辑代码处理等都，都需要自己手动封装完成；
- 并且所有的代码逻辑都放在一起，会非常的混乱；

目前在 Node 中，比较流行的 Web 服务器框架是 express、koa；

express 早于 koa 出现，并且在 Node 社区中迅速流行起来：
- 基于 express 可快速、方便的开发自己的 Web 服务器；
- 并且可以通过一些实用工具和中间件来扩展功能；

> 【注意】：Express 整个框架的核心，就是中间件，理解了中间件其他一切都非常简单！
...
使用 express，可以方便的将请求进行分离；无论是不同的 URL，还是 get、post 等请求方式；方便对进行维护、扩展；
...
返回数据，我们可以方便的使用 json：
- res.json(数据)方式；
- 可以支持其他的方式，可以自行[查看文档](https://www.expressjs.com.cn/guide/routing.html)；

# 六、Express 中间件

Express 是一个路由和中间件组成的 Web 框架，它本身的功能非常少：
...
如果当前中间件功能没有结束请求-响应周期，则必须调用 `next` 函数，将控制权传递给下一个中间件功能，否则，请求将被挂起。
...
express 主要提供了两种方式，将一个中间件应用到我们的应用程序中：可以是 `app` 也可以是 `router`；
